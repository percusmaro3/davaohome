...
function drop (ev) {
	...
	if (typeof ev.dataTransfer.files == 'undefined') {
		...
		if (!!files && nCount === 0) {
			...
			alert("It’s not right way to attach.");
			return;
		}

		for (var i = 0, j = nImageFileCount; i < nCount; i++) {
			if (!rFilter.test(files[i].type)) {
				alert("Only (png, gif, and jpg bmp) files can be uploaded.");
			} else if (files[i].size > nMaxImageSize) {
				alert("Image can’t be registered by capacity of more than 1 in mb.");
			} else {
				//제한된 수만 업로드 가능.
				if (j < nMaxImageCount) {
...
function addImage (ofile) {
	...
	if (!!bExceedLimitTotalSize) {
		alert("It can’t register as exceed capacity 50MB. \n\n (File name : " + sFileName + ", Size : " + sFileSize + ")");
	} else {
		//이미지 정보 저장
		...
		aFileList.push('	<a onclick="delImage(\'img' + nImageInfoCnt + '\')"><img class="del_button" src="../../img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="Delete attached picture"></a>');
		aFileList.push('	</li> ');
...
function html5Upload () {
	...
	sUploadURL = '/common/updatePhotoMulti.nhn'; 	//upload URL
		...
		onload: function (res) { // 요청이 완료되면 실행될 콜백 함수
			...
			if (res.readyState() == 4) {
				if (sResString.indexOf("NOTALLOW_") > -1) {
					...
					alert("Only (png, gif, and jpg bmp) files can be uploaded. (" + sFileName + ")");
				} else {
...
function callFileUploader () {
	oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
		sUrl: '/common/updatePhoto.nhn',
		sCallback: '/resources/smartEdit/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
